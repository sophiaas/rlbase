...
#             if len(dim2_selection.shape) > 1:
            for i in range(x.shape[0]):
                out[i] = torch.matmul(x[i], self.weight[dim2_selection[i]]) + self.bias[dim2_selection[i]]
        else:
...
#     def __init__(self, in_features, in_features_1, out_features):
#         # in_features_0 should be obs_dim or hdim
#         # in_features_1 should be num_options
#         # out_features should be hdim or num_actions
#         super(Linear3D, self).__init__()
#         self.in_features = in_features
#         self.in_features_1 = in_features_1
#         self.out_features = out_features
#         self.weight = Parameter(torch.Tensor(in_features_1, in_features, 
#                                              out_features))
#         self.bias = Parameter(torch.Tensor(in_features_1, out_features))
#         self.reset_parameters()
...
#     def forward(self, input, dim1_selection=None):
#         if input.shape[0] > 1:
#             out = torch.zeros((input.shape[0], self.weight.shape[2]))
#             for i in range(input.shape[0]):
#                 out[i] = torch.matmul(input[i], self.weight[dim1_selection[i]]) + self.bias[dim1_selection[i]]
#         else:
#             W = self.weight[dim1_selection]   
#             b = self.bias[dim1_selection] 
