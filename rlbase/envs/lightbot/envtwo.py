...
from .maps import puzzles
from .maps.map_parsing import extract_map_features
import torch
...
#         """
        self.config = config
        self.board = puzzles.maps[self.config.puzzle_name]
        self.map = self.board['map']
        self.extract_map_features(self.map)
...
#         self.h_actions = {a:[] for a in range(self.n_actions, 
#                                               self.num_h_actions+self.n_actions)}
#         self.open_h_action_index = self.n_actions
    ...
    def extract_map_features(self):
        ...
        coords = list(zip(x, y))
        self.board_properties = {a: {"height": b, "light_idx": c} for a, b, c 
                                 in zip(coords, height, light_idx)}
...
#         self.raw_state, self.reward, self.done, frame_update = copy.deepcopy(self.update(action))
        ...
        if self.frame_count >= self.max_count:
            self.done = True 
        return copy.deepcopy(self.raw_state), copy.deepcopy(self.reward), copy.deepcopy(self.done), frame_update
